...
def main(d_var):
    ...
    for updating_window in updating_windows:
        ...
        for d_var in d_vars:
            ...
            diff[d_var] = {'const_diff':constant_diff_list,
                           'base_diff':base_diff_list,
                           'text_diff':text_diff_list,
                           'full_diff':full_diff_list}
    ...
    return rmse_df, rmse_series, prediction, diff
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for result in results:
            ...
            diff_series.update(result[3])

    prediction_series = pd.DataFrame(prediction_series)            
    ...
    prediction_series.to_excel('/user/sk5193/files/Energy/outputs/wipimom_updated/new_variables/parsimonious/'
                    +file+'/'+file_start+'/Lasso_prediction_'+str(cv_fold)+'fold_'+file_suffix+'_t.xlsx')
    diff_series.to_excel('/user/sk5193/files/Energy/outputs/wipimom_updated/new_variables/parsimonious/'
                    +file+'/'+file_start+'/Lasso_diff_'+str(cv_fold)+'fold_'+file_suffix+'_t.xlsx')