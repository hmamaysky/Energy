...
dir <- "/Users/billwu/Dropbox/ncm_research/Monte Carlo/"
setwd(dir)
...
data.prices.pca <- read.dta13("/Users/billwu/Dropbox/ncm_research/forwardSelection/transformed_data_prices_pca_v11.1.dta") 
...
toLag <- c("FutRet", "DSpot", "DOilVol", "xomRet", "bpRet", "rdsaRet", "OilVol", 
            ...
            "fEpg", "sBbl", "fBbl", "sRpc", "fRpc", "sEp", "fEp", "vix", "vix_spx", 
...
MC.FS <- function(subset, seed, horizon, title, sims){
  ...
  results$depVar.lagDepVar <- lm(subset[,1] ~ lag(subset[,2], horizon), data = subset)
  ...
  if(horizon == 4){
    AR <- lm(final.df[[1]] ~ 
               lag(final.df[[1]], n = 1) + lag(final.df[[1]], n = 2) + lag(final.df[[1]], n = 3) + lag(final.df[[1]], n = 4),
             data = final.df)
    
  } else if(horizon == 8){
    AR <- lm(final.df[[1]] ~ 
               lag(final.df[[1]], n = 1) + lag(final.df[[1]], n = 2) + lag(final.df[[1]], n = 3) + lag(final.df[[1]], n = 4) +
               lag(final.df[[1]], n = 5) + lag(final.df[[1]], n = 6) + lag(final.df[[1]], n = 7) + lag(final.df[[1]], n = 8),
             data = final.df)
  ...
  y_resid <- final.df[[1]] - y_hat
  var_e <- var(na.omit(y_resid))
  ...
  data.rsq <- as.data.frame(rsq.sims)
  dens.rsq <- melt(data.rsq)
  
  data.tstat <- as.data.frame(tstat.sims)
...
subset <- dplyr::select(detrended.prices, 
                 c(FutRet_t4, 
                   ...
                   fRpc, sEp, fEp, vix, vix_spx, PCAsent, PCAfreq, PCAall))
...
tab_model(FutRet_8wk$sdcoef, DSpot_8wk$sdcoef, DOilVol_8wk$sdcoef, xomRet_8wk$sdcoef, bpRet_8wk$sdcoef, rdsaRet_8wk$sdcoef, DInv_8wk$sdcoef, DProd_8wk$sdcoef, 
          ...
          vcov.fun = "vcovHAC", file = "20201102/forwardSelection_8wks_allVars_new.doc")
...
tab_model(FutRet_8wk$depVar.lagDepVar, DSpot_8wk$depVar.lagDepVar, DOilVol_8wk$depVar.lagDepVar, xomRet_8wk$depVar.lagDepVar, 
          ...
          vcov.fun = "vcovHAC", file = "20201102/forwardSelection_8wks_AR1_new.doc")
...
tab_model(FutRet_4wk$sdcoef, DSpot_4wk$sdcoef, DOilVol_4wk$sdcoef, xomRet_4wk$sdcoef, bpRet_4wk$sdcoef, rdsaRet_4wk$sdcoef, DInv_4wk$sdcoef, DProd_4wk$sdcoef, 
          show.ci = F, show.p = F, p.style = "star",  title = "Stepwise Forward Selection Regressions at the 8-week Horizon",
          vcov.fun = "vcovHAC", file = "20201102/forwardSelection_4wks_allVars_new.doc")
...
tab_model(FutRet_4wk$depVar.lagDepVar, DSpot_4wk$depVar.lagDepVar, DOilVol_4wk$depVar.lagDepVar, xomRet_4wk$depVar.lagDepVar, 
          ...
          show.ci = F, show.p = F, p.style = "star",  title = "Dependent variables on lagged dependent variable coefficients",
          vcov.fun = "vcovHAC", file = "20201102/forwardSelection_4wks_AR1_new.doc")

# Adjusted R-Squared
pdf("20201102/Monte_Carlo_Exhibits_fs_rsq_new.pdf", paper = "letter", width = 7.5, height = 10, family = "Times")
...
# T-Statistics
pdf("20201102/Monte_Carlo_Exhibits_fs_tstat_new.pdf", paper = "letter", width = 7.5, height = 10, family = "Times")
...
pdf("20201102/Fig5.pdf", paper = "letter", width = 7.5, height = 10, family = "Times")
...
pdf("20201102/Fig4.pdf", paper = "letter", width = 7.5, height = 10, family = "Times")
